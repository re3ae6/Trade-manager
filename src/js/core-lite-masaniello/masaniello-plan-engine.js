...
/**
 * Masaniello-Lite:
 * Small-account planning path using the same Masaniello engine,
 * but with a bounded compact N/K search.
 *
 * It never replaces a normal Masaniello plan.
 */
export function buildLitePlan({
  capital,
  payoutPct,
  targetProfit,
  floor = 1,
  ...
  for (let n = 1; n <= Math.min(maxN, 300); n++) {
    for (let k = 1; k <= n; k++) {
      ...
      if (
        !Number.isFinite(worstCaseFinal) ||
        worstCaseFinal + 1e-9 < targetBalance
      ) {
        continue;
      }
      ...
      while (nRemaining > 0 && kRemaining > 0) {
        ...
        if (result.reason !== 'ok' || result.stake <= 0) {
          valid = false;
          break;
        }

        preview.push(result.stake);

        // Worst-case preview: losses first, then required wins.
        if (preview.length <= n - k) {
          balance -= result.stake;
        } else {
          balance += result.stake * (Q - 1);
          kRemaining--;
        }
      ...
      return {
        ...
        worstCaseFinal,
        floor,
        maxStake: Math.max(...preview),
        stakesPreview: preview
      };
    }
  }
