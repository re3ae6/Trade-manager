import { computePlan } from './masaniello-plan-engine.js';
...
export function computeTradingPlanStats(tradingPlan, sessionHistory, currentBalance){
  if(!tradingPlan){
    return {remainingProfit:null,progressPercent:null,sessionsCompletedTowardPlan:0,averageProfitPerCompletedSession:null,estimatedSessionsRemaining:null,targetReached:false};
  }
  ...
  // requiredAverageProfitPerSession removed (Phase 15 / P3, Audit finding #7):
  // it algebraically reduced to remainingProfit / (remainingProfit / averageProfitPerCompletedSession)
  // === averageProfitPerCompletedSession, so it was always identical to that field, was never
  // consumed by the UI, and had no independent input. See TRADE_MANAGER_AUDIT_LOCKED_FIX_PLAN §17.
  return {remainingProfit,progressPercent,sessionsCompletedTowardPlan,averageProfitPerCompletedSession,estimatedSessionsRemaining,targetReached:progressPercent>=100};
}
